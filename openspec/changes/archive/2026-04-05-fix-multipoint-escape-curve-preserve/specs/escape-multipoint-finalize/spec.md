...
- **WHEN** the user selects the arrow tool, clicks to place a first point, clicks to place a second point, and then presses Escape (pointer is up)
...
The Escape-to-cancel behavior for single-segment pointer-drag creation (rectangles, ellipses, diamonds, freedraw, single-segment linear elements) SHALL continue to work as before — cancelling the element and removing it from the scene.
